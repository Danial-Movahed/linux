	...
	for (i = 0; i < 5 && buf[6+i] != 0x00; ++i) {
		...
		if (!found) {
			...
			if (nanosic_dev->caps_as_second_layer_key && keycode == KEY_CAPSLOCK) {
				...
				nanosic_dev->second_layer_key_pressed = true;
				continue;
			}
			if (nanosic_dev->second_layer_key_pressed)
	...
	for (i = 0; i < 5 && nanosic_dev->last_pressed_key[i] != 0x00; ++i) {
		...
		if (!found) {
			...
			if (nanosic_dev->caps_as_second_layer_key && keycode == KEY_CAPSLOCK) {
				...
				nanosic_dev->second_layer_key_pressed = false;
				continue;
			}
			if (nanosic_dev->second_layer_key_pressed)
				keycode = second_layer_to_linux_keycode[(unsigned char)nanosic_dev->last_pressed_key[i]];
